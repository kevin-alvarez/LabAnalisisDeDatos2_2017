...
EstudioLimpiado <- EstudioLimpiado[-indicesAEliminar, ,drop=F]
  ...
  # Se eliminan los registros con valores fuera de lo común o imposibles.
...
EstudioLimpiado <- subset(EstudioLimpiado, (as.numeric(as.character(TSH)) <= TSHMAX) & (as.numeric(as.character(T3)) <= T3MAX) & (as.numeric(as.character(TT4)) <= TT4MAX) & (as.numeric(as.character(T4U)) <= T4UMAX))
  ...
  # Se realiza una transformación de los datos al tipo correspondiente en R.
...
sil_width <- c(NA)
  ...
  # Metodo de la silueta para obtener el k optimo para k-medoids
for(i in 2:10){
...
lines(1:10, sil_width)
  ...
  # Calculo de clustering para k=3,4 y 5
clust3 <- pam(gower_dist, diss = TRUE, k = 3)
...
tsne_data <- tsne_obj$Y %>%
  data.frame() %>%
  setNames(c("X", "Y")) %>%
  mutate(cluster = factor(clust3$clustering))

ggplot(aes(x = X, y = Y), data = tsne_data) +
  geom_point(aes(color = cluster)) + ggtitle("3-medoids clustering")

clusplot(Estudio2, clust3$clustering, color=TRUE, shade=T, lines=0)
...
ggplot(aes(x = X, y = Y), data = tsne_data) +
  geom_point(aes(color = cluster)) + ggtitle("4-medoids clustering")

clusplot(Estudio2, clust4$clustering, color=TRUE, shade=T, lines=0)
...
tsne_data <- tsne_obj$Y %>%
  data.frame() %>%
  setNames(c("X", "Y")) %>%
  mutate(cluster = factor(clust5$clustering))

ggplot(aes(x = X, y = Y), data = tsne_data) +
  geom_point(aes(color = cluster)) + ggtitle("5-medoids clustering")

clusplot(Estudio2, clust5$clustering, color=TRUE, shade=T, lines=0)

### ANALISIS DEL CLUSTER 5

pam_results <- Estudio2 %>%
  mutate(cluster = clust5$clustering) %>%
  group_by(cluster) %>%
